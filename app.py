...
#ngrok.set_auth_token(2SI7NsTjXoF0RBYx4OsvqpuVGDn_7WVQAAzfERs6cyLT7MhfB)
...
def fetch_movie_details(imdb_id)
    if imdb_id
        imdb_id_str = ftt{str(imdb_id).zfill(7)}
        api_key = 'd0b8676c'
        url = f'httpwww.omdbapi.comi={imdb_id_str}&apikey={api_key}'
        response = requests.get(url)
        if response.status_code == 200
            data = response.json()
            if data['Response'] == 'True'
                title = data.get('Title', 'NA')
                release_date = data.get('Released', 'NA')
                overview = data.get('Plot', 'NA')
                poster_path = data.get('Poster', 'NA')
                return {'title' title, 'release_date' release_date, 'overview' overview, 'poster_path' poster_path}
    return {'title' 'NA', 'release_date' 'NA', 'overview' 'NA', 'poster_path' 'NA'}
                ...
                [
                    ...
                    dbc.CardBody(
                        [
                            ...
                            html.P(fRelease Date {details['release_date']}, className='card-text',style={'font-size' '12px'}),
